  ...
  return players.slice(1)
    .reduce((promise, player) => {
      ...
      return promise.then(() => player.setAVTransport(groupingUri))
        .catch(err => {
          logger.warn(`Failed to add ${player.roomName} to group with ${coordinator.roomName}: ${err.message}`);
        });
    }, initialPromise);
}
...
function ungroupFromCoordinator(system, players) {
  ...
  return superfluousPlayers.reduce((promise, player) => {
    ...
    return promise.then(() => player.becomeCoordinatorOfStandaloneGroup())
      .catch(err => {
        logger.warn(`Failed to ungroup ${player.roomName} from ${coordinator.roomName}: ${err.message}`);
      });
  }, Promise.resolve());
  ...
  promise = promise.then(() => {
    ...
    if (coordinator.coordinator.uuid !== coordinator.uuid) {
      ...
      return coordinator.becomeCoordinatorOfStandaloneGroup()
        .catch(err => {
          logger.warn(`Failed to break out coordinator ${coordinator.roomName}: ${err.message}`);
          throw err; // Re-throw as this is critical for preset to work
        })
